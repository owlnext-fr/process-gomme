...
| Énergie par étage dans l'immeuble | 2026-06-14 | — | — | ✅ Livré | Chaque ligne de la liste « Ton immeuble » (`Synthese.tsx`) affiche `Nom — X% — <phrase ressource>`. Phrases originales dans `src/content/energie.ts` (`ENERGIE: Record<TypeId,string>`, ancrées sur `essenceBase`, nuance générique du coût haut/bas). Testé (`energie.test.ts`). Commit `b47759d` |
