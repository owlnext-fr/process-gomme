...
- **Prévisualiser un PDF (`@react-pdf/renderer`)** : `pdftoppm` est installé (`/usr/bin/pdftoppm`, paquet poppler ; `gs` aussi). Recette : test jetable `// @vitest-environment node` qui fait `renderToFile(<ResultPdfDocument .../>, "/tmp/x.pdf")`, lancé via `node_modules/.bin/vitest run <fichier>`, puis `pdftoppm -png -r 110 /tmp/x.pdf /tmp/x` → lire `/tmp/x-1.png`. Supprimer le test jetable + les `/tmp` avant commit. Voir QUIRKS.
